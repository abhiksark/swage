...
| M9: Persistent scheduling | Device task queue, resident CTAs, dependency handling | Correct under extreme skew; reduces long-tail idle on a predeclared benchmark | **In progress**; private identity-sum claims and split completion are implemented, while the predeclared performance gate has no committed result yet |
