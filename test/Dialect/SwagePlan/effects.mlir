...
module {
  ...
  func.func @classify_around_offsets_write(
    ...
    %before = swage_plan.classify %offsets, %value_count, %segment_count {kernel = @semantic_sum, policies = [#swage_plan.policy<warp>, #swage_plan.policy<cta>], warp_max_elements = 32 : i32} : memref<?xi32>, i32, i32 -> !swage_plan.task_range
    %c0 = arith.constant 0 : index
    // CHECK: memref.store
    memref.store %value_count, %offsets[%c0] : memref<?xi32>
    // CHECK: %[[AFTER:.*]] = swage_plan.classify
    %after = swage_plan.classify %offsets, %value_count, %segment_count {kernel = @semantic_sum, policies = [#swage_plan.policy<warp>, #swage_plan.policy<cta>], warp_max_elements = 32 : i32} : memref<?xi32>, i32, i32 -> !swage_plan.task_range
